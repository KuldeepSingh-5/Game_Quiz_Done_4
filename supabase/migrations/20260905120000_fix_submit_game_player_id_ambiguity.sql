-- Finalize the game-result save RPC with qualified table references.
-- The RETURNS TABLE player_id name must not collide with table columns.
...
BEGIN
  ...
  INSERT INTO public.game_results AS gr
    (player_id, score, correct_answers, attempted, xp_earned, date)
  VALUES
    (v_player_id, v_score, p_correct, p_attempted, v_xp_earned, v_today)
  RETURNING gr.id INTO v_game_result_id;
