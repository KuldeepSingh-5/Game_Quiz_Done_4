...
## Problem
The function stored options as a text[] under the key 'options' in the JSONB
object, but the INSERT statement read option_a, option_b, option_c, option_d
from the JSONB — which don't exist. This caused all inserts to produce NULL
options, violating the NOT NULL constraint.

## Fix
Store each option as a separate key (option_a, option_b, option_c, option_d)
in the JSONB object so the INSERT can read them correctly.

## Security
- No new tables, no policy changes
- SECURITY DEFINER with search_path = 'public' preserved
- Admin permission check preserved
*/
...
GRANT EXECUTE ON FUNCTION public.admin_bulk_create_questions(jsonb) TO anon;
